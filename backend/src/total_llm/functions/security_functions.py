"""
Qwen3.5 Function Calling Definitions for Security Monitoring System
...
- Qwen3.5의 네이티브 tool calling을 사용하여 함수 호출을 정확히 수행합니다.
- 동일 모델에서 비전 입력도 처리 가능하므로 이미지 맥락이 제공되면 함께 반영합니다.
- 필요 시 thinking 모드를 사용해 중간 추론을 강화하되 최종 답변은 간결하게 제공합니다.
...
- Qwen3.5 네이티브 tool calling을 사용하며 별도 함수 스키마 우회 규칙은 필요하지 않습니다.
...
- Qwen3.5 thinking 모드를 사용할 수 있지만, 실행 지시는 항상 명확한 함수 호출로 마무리합니다.
