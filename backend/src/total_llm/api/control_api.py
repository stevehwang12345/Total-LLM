...
from total_llm.services.control.function_schemas import ALL_FUNCTIONS, ACU_FUNCTIONS, CCTV_FUNCTIONS
from total_llm.services.control.network_discovery import (
    NetworkDiscoveryService,
    DeviceType,
    DeviceStatus,
    get_discovery_service,
)
from total_llm.services.control.device_registry import (
    DeviceRegistry,
    RegisteredDevice,
    ConnectionStatus,
)
from total_llm.core.dependencies import (
    ControlAuditLoggerDep,
    ControlCredentialManagerDep,
    ControlDeviceRegistryDep,
    ControlHealthServiceDep,
    ControlNetworkDiscoveryDep,
    ControlRateLimiterDep,
    ControlSystemControllerDep,
    ControlZoneManagerDep,
    SettingsDep,
)
...
async def process_command(
    ...
    controller: ControlSystemControllerDep = None,
):
    """
...
async def get_door_status(
    door_id: Optional[str] = Query(None, description="출입문 ID (없으면 전체)"),
    controller: ControlSystemControllerDep = None,
):
    """출입문 상태를 조회합니다."""
...
async def get_camera_status(
    camera_id: Optional[str] = Query(None, description="카메라 ID (없으면 전체)"),
    controller: ControlSystemControllerDep = None,
):
    """카메라 상태를 조회합니다."""
...
async def get_system_status(controller: ControlSystemControllerDep = None):
...
async def health_check(
    controller: ControlSystemControllerDep = None,
    registry: ControlDeviceRegistryDep = None,
):
...
async def scan_network(
    ...
    discovery: ControlNetworkDiscoveryDep = None,
):
    """
...
async def register_device(
    ...
    try:
        ...
        # LLM 제어 시스템 갱신
        await _sync_devices_to_controller(registry, controller)
...
async def _sync_devices_to_controller(
    registry: ControlDeviceRegistryDep,
    controller: ControlSystemControllerDep,
):
    """
    ...
    """
    from total_llm.services.control.cctv_controller import CameraInfo, CameraState, PTZPosition
    from total_llm.services.control.acu_controller import DoorInfo, DoorState
...
async def export_devices_for_api(registry: ControlDeviceRegistryDep = None):
    """
...
async def get_encryption_status(manager: ControlCredentialManagerDep = None):
    """
...
from total_llm.services.control.connection_health import (
...
async def get_device_health(
    device_id: str,
    registry: ControlDeviceRegistryDep = None,
    health_service: ControlHealthServiceDep = None,
):
    """
...
async def diagnose_device_connection(
    device_id: str,
...
async def reconnect_device(
    device_id: str,
    request: ReconnectRequest | None = None,
    registry: ControlDeviceRegistryDep = None,
    health_service: ControlHealthServiceDep = None,
    controller: ControlSystemControllerDep = None,
):
...
async def get_connection_stats(health_service: ControlHealthServiceDep = None):
    """
    모든 장치의 연결 통계를 조회합니다.
...
from total_llm.services.control.zone_manager import (
    ZoneManager,
    Zone,
    SecurityLevel,
    get_zone_manager,
...
async def list_zones(zone_manager: ControlZoneManagerDep = None):
    """
    등록된 모든 존 목록을 조회합니다.
...
async def get_zone_hierarchy(zone_manager: ControlZoneManagerDep = None):
    """
...
async def get_zone(zone_id: str, zone_manager: ControlZoneManagerDep = None):
    """특정 존의 상세 정보를 조회합니다."""
...
async def create_zone(request: ZoneCreateRequest, zone_manager: ControlZoneManagerDep = None):
    """
    새로운 존을 생성합니다.
...
async def update_zone(
    ...
    zone_manager: ControlZoneManagerDep = None,
):
    """존 정보를 수정합니다."""
    security_level = None
...
async def delete_zone(
    zone_id: str,
    force: bool = Query(False, description="강제 삭제 (하위 존/장치 포함)"),
    zone_manager: ControlZoneManagerDep = None,
):
    """
    존을 삭제합니다.
...
async def assign_device_to_zone(
    device_id: str,
    request: DeviceZoneAssignRequest,
    zone_manager: ControlZoneManagerDep = None,
    registry: ControlDeviceRegistryDep = None,
    audit: ControlAuditLoggerDep = None,
):
    """
    장치를 특정 존에 배정합니다.
...
from total_llm.services.control.audit_logger import (
    AuditLogger,
    AuditEventType,
    AuditSeverity,
    get_audit_logger,
...
async def get_security_logs(
    ...
    security_events = [
        ...
        AuditEventType.CREDENTIAL_DECRYPT,
    ]
...
from total_llm.services.control.rate_limiter import (
    RateLimiter,
    RateLimitType,
    get_rate_limiter,
...
async def get_rate_limit_status(
    ...
    limiter: ControlRateLimiterDep = None,
):
    """
...
async def get_rate_limit_stats(limiter: ControlRateLimiterDep = None):
    """전체 속도 제한 통계를 조회합니다."""
...
async def cleanup_rate_limits(limiter: ControlRateLimiterDep = None):
    """만료된 속도 제한 상태를 정리합니다."""
...
async def detailed_health_check(
    ...
    # 5. vLLM 서버 연결 확인
    vllm_url = settings.llm.base_url
...
async def check_service_connections(settings: SettingsDep):
    """
